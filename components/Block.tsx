...
import { openSafeUrl, isValidYouTubeChannelId, isValidLocationString, isValidSafeUrl } from '../utils/security';
    ...
    >
      ...
      >
        ...
        <div className="w-full h-full pointer-events-none relative z-10">
          ...
          {block.type === BlockType.MEDIA && block.imageUrl && !isLinkWithImage ? (
          ...
          ) : block.type === BlockType.MAP_EMBED ? (
            /* MAP_EMBED BLOCK - Google Maps embed with directions button */
            <div className="w-full h-full relative bg-gray-100 overflow-hidden flex flex-col">
              {/* Map iframe */}
              <div className="flex-1 relative">
                {block.mapEmbedUrl && isValidSafeUrl(block.mapEmbedUrl) ? (
                  <iframe
                    width="100%"
                    height="100%"
                    className="grayscale-[10%] group-hover:grayscale-0 transition-all duration-500"
                  ...
                  <iframe
                    ...
                    height="100%"
                    className="grayscale-[10%] group-hover:grayscale-0 transition-all duration-500"
                    src={`https://maps.google.com/maps?q=${encodeURIComponent(block.mapAddress)}&z=${block.mapZoom || 15}&output=embed`}
                    loading="lazy"
                    sandbox="allow-scripts allow-same-origin"
                  ></iframe>
                ) : (
                  <div className="w-full h-full flex items-center justify-center text-gray-400">
                    <div className="text-center">
                      ...
                      <span className="text-xs">Add address or embed URL</span>
                    </div>
                  </div>
                )}
              </div>
              {/* Bottom section with title and directions button */}
              <div className="absolute bottom-0 left-0 right-0 p-2 md:p-3 bg-gradient-to-t from-black/70 via-black/40 to-transparent">
                {block.title && (
                  <p className={`font-semibold text-white drop-shadow mb-2 ${textSizes.overlayTitle}`}>
                    {block.title}
                  </p>
                )}
                {block.mapShowDirections !== false && (block.mapAddress || block.mapEmbedUrl) && (
                  <a
                    href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(block.mapAddress || '')}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    onClick={(e) => e.stopPropagation()}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white text-gray-900 rounded-full text-xs font-semibold hover:bg-gray-100 transition-colors shadow-md"
                  >
                    <Navigation size={12} />
                    Get Directions
                  </a>
            ...
            /* RATING BLOCK - Google rating/review embed */
            <div className="w-full h-full relative flex flex-col items-center justify-center p-3 md:p-4">
              {/* Star rating display */}
              ...
              <div className="text-center">
                <span className="text-2xl md:text-3xl font-bold text-gray-900">
                  {block.ratingValue?.toFixed(1) || '0.0'}
                </span>
                {block.ratingCount !== undefined && (
                  <p className="text-xs text-gray-500 mt-1">
                    ({block.ratingCount.toLocaleString()} reviews)
                  </p>
                )}
              </div>
              {/* Google badge */}
              <div className="flex items-center gap-1.5 mt-3 px-3 py-1.5 bg-gray-100 rounded-full">
              ...
              {/* Title */}
              {block.title && (
                <p className={`mt-3 font-medium text-gray-700 text-center ${textSizes.overlayTitle}`}>
                  {block.title}
                </p>
              )}
              {/* Subtext */}
              {block.subtext && (
                <p className={`mt-1 text-gray-500 text-center ${textSizes.overlaySubtext}`}>
                  {block.subtext}
                </p>
              )}
            </div>
          ) : block.type === BlockType.QR_CODE ? (
            /* QR_CODE BLOCK - QR code generator with download */
            <div className="w-full h-full relative flex flex-col items-center justify-center p-3 md:p-4">
              {/* QR Code */}
              <div className="bg-white p-2 md:p-3 rounded-xl shadow-sm">
                <QRCodeSVG
                  value={block.qrContent || window.location.origin}
                  size={Math.min(block.colSpan, block.rowSpan) <= 2 ? 80 : 120}
                  level="M"
                  bgColor="#ffffff"
                  fgColor="#000000"
                  includeMargin={false}
              ...
              {/* Title */}
              {block.title && (
                <p className={`mt-2 font-semibold text-center ${textSizes.overlayTitle}`}>
                  {block.title}
                </p>
              )}
              {/* Download button */}
              {block.qrShowDownload !== false && (
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                  ...
                  className="mt-3 inline-flex items-center gap-1.5 px-3 py-1.5 bg-gray-900 text-white rounded-full text-xs font-semibold hover:bg-gray-800 transition-colors"
                >
                  <Download size={12} />
                  Download QR
                </button>
