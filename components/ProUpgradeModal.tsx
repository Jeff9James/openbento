...
import { createCheckoutSession, PRICING, isDodoPaymentsConfigured } from '../lib/dodoPayments';
...
export default function ProUpgradeModal({ isOpen, onClose }: ProUpgradeModalProps) {
  ...
  const dodoConfigured = isDodoPaymentsConfigured();
  ...
  const handleUpgrade = async () => {
    ...
    try {
      ...
      const session = await createCheckoutSession(productId, user?.email, user?.id);
          ...
          <div className="p-6">
            ...
            {!dodoConfigured && (
              <div className="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-xl text-blue-800 text-sm">
                DodoPayments payment system is being configured. Contact support to upgrade.
              </div>
            )}
